use anyhow::Result;
use orunla::graph::{GraphStore, Node, NodeType};
...
impl MCPServer {
    ...
    pub async fn handle_message(&self, msg: &MCPMessage) -> Result<Option<serde_json::Value>> {
        let response = match msg.method.as_str() {
            ...
            // Ignore notifications/unknown methods without sending a response
            _ => return Ok(None),
        };

        // If it's a notification (no id), we shouldn't send a response anyway, 
        // but our handle_message logic now handles it via the match above.
    ...
    async fn tool_memory_add(&self, args: &serde_json::Value) -> Result<serde_json::Value> {
        ...
        let edge =
            orunla::graph::Edge::new(s_id, o_id, input.predicate.clone(), input.text.clone());
    ...
    async fn tool_memory_get_context(&self, args: &serde_json::Value) -> Result<serde_json::Value> {
        ...
        let request = RecallRequest {
            ...
            limit: 10,
            min_confidence: 0.0,
            min_strength: 0.1, // Default threshold for context retrieval
        };
        let response = retriever.recall(request)?;
    ...
    async fn tool_memory_sync_chat(&self, args: &serde_json::Value) -> Result<serde_json::Value> {
        ...
        for triplet in triplets {
            ...
            let edge = orunla::graph::Edge::new(s_id, o_id, triplet.predicate, source_text);
...
async fn main() -> Result<()> {
    ...
    let tier = LicenseValidator::get_tier_local(&license_store).unwrap_or(orunla::licensing::Tier::Free);
    let sync_status = if tier.allows_sync() { "enabled" } else { "disabled" };
